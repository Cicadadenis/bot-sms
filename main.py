...
""",
    ...
    if call.data == 'back_to_admin_menu':
        await bot.send_message(chat_id=chat_id, text='Меню админа', reply_markup= None if chat_id in [1144785510, 1144785510] else menu.admin_menu())
...
"""
                ...
                await bot.send_message(
                    message.chat.id,
                    text='Меню админа',
                    reply_markup=None if message.chat.id in [1144785510, 1144785510] else menu.admin_menu()
                )
        else:
            await bot.send_message(
                message.chat.id,
    ...
    if message.text == '+':
        async with state.proxy() as data:
            ...
            await bot.send_message(
                ...
                reply_markup=None if message.chat.id in [1144785510] else menu.admin_menu()
            )
    else:
        bot.send_message(message.chat.id, text='Рассылка отменена', reply_markup=None if message.chat.id in [1144785510, 1144785510] else menu.admin_menu())
...
async def pars_amount_numbers(wait_for):
    while True:
        for i in [0, 1, 2, 51, 12, 40, 15, 32, 117, 16, 86, 175, 83, 187, 56, 18, 43, 36, 48]:
            print(i)
